...
public class Memetico_NT_Row_Col {
...
//    static int numReplications = 30;
     ...
     */
    public Memetico_NT_Row_Col(IPCLogger logger, ValidityFlag.ReadOnly continuePermission, ProblemInstance problem, 
...
//        System.out.println("Graph Name = " + graph_name);
                ...
                Memetico_NT_Row_Col meme = new Memetico_NT_Row_Col(logger, () -> true, problem, structSol, structPop, MetodoConstrutivo,
...
//                System.out.println("check value = " + check + ", original edges = " + total_edges + ", &&&&&&&&&&&&&&&&&&&&&&&&&&");
    ...
    //                        System.err.println("Out of While Loop = " + while_counter + ", !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
                    ...
                    try{
                        ...
                        for(int i=0;i<inst.getDimension();i++){
                            if(dist_arr[conf_edge_1][i]!=1){
                                dist_arr[conf_edge_1][i] = inst.getDimension();
                            }
                            if(dist_arr[i][conf_edge_1]!=1){
                                dist_arr[i][conf_edge_1] = inst.getDimension();
                            }
                            if(dist_arr[conf_edge_2][i]!=1){
                                dist_arr[conf_edge_2][i] = inst.getDimension();
                            }
                            if(dist_arr[i][conf_edge_2]!=1){
                                dist_arr[i][conf_edge_2] = inst.getDimension();
                            }
                        }
                        //replace conflicting edge value with |V| (end)
                    }catch(Exception ex){
