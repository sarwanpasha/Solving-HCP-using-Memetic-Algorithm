...
public class Memetico_TC_with_Poland {
...
//    static int numReplications = 30;
     ...
     */
    public Memetico_TC_with_Poland(IPCLogger logger, ValidityFlag.ReadOnly continuePermission, ProblemInstance problem, 
            ...
            int orig_length,int true_cost,int instance_ID/*, DataOutputStream fileOut, DataOutputStream compact_fileOut*/) throws Exception {
...
//            System.err.println("refConstr = " + refConstr);
            ...
            // Polish group logic (start)
            int instance_number = instance_ID;
            ...
            if(tour.length!=1){
                ...
                double[][] pol_dist_mat = array_to_matrix_first_time_TC(tour.length,tour,((GraphInstance) inst).getMatDist());
                ((GraphInstance) inst).setMatDist(pol_dist_mat);
                
//                memePop.evaluatePop(inst);
//                double[][] test4 = ((GraphInstance) inst).getMatDist();
//                for(int u=0;u<tour.length;u++){
//                    for(int j=0;j<tour.length;j++){
//                        System.out.print((int)pol_dist_mat[u][j] + " ");
//                    }
//                    System.out.println();
//                }
//                System.out.println("TOTAL ONES in original matrix = " + ones_counter_fun_dist_mat(pol_dist_mat, inst.getDimension()) 
//                + "@@@@@@@@@@@@@@@@@@@");
                
                int conf_finder_inst_1[][] = conflicting_edge_finder(tour,pol_dist_mat);
...
//                memePop.updateAgents(inst);
                ...
                for (int ii = 0; ii < memePop.popSize; ii++) {
                    refMut.runMutation(pocCurPop[ii].current);
                    pocCurPop[ii].current.calculateCost(inst);
                    refLocalSearch.runLocalSearch(pocCurPop[ii].current, inst);
                }

                for (int ii = 1; ii < memePop.popSize; ii++) {
                    refMut.runMutation(pocCurPop[ii].pocket);
                    pocCurPop[ii].pocket.calculateCost(inst);
                    refLocalSearch.runLocalSearch(pocCurPop[ii].pocket, inst);
                }
//                SolutionStructure tmp = pocCurPop[0].current.deepCopy();
//                lkh.runLocalSearch(tmp, inst);
//                pocCurPop[0].current.calculateCost(inst);
//                pocCurPop[0].insertSolutionStructure(pocCurPop[0].current);
//                pocCurPop[0].updateAgent(inst);
                
//                bestSolution = memePop.bestSolution;
                System.err.println("tmp.current = " + pocCurPop[0].current.cost + ", bestSolution = " + memePop.bestSolution.cost);
//                System.err.println("tmp.pocket = " + pocCurPop[0].pocket.cost);
            }else{
                System.out.println("Not Solved by polish people");
        ...
        // Polish group logic (end)
            
//            double[][] test4 = ((GraphInstance) inst).getMatDist();
//                for(int u=0;u<tour.length;u++){
//                    for(int j=0;j<tour.length;j++){
//                        System.out.print((int)test4[u][j] + " ");
//                    }
//                    System.out.println();
//                }
...
TotalTime = (System.currentTimeMillis() - TotalTime);
   ...
   Final_Best_solution_Length = memePop.bestAgent.bestCost;
       ...
       Final_Best_solution_Length =inst.getDimension() ;
   }
...
//        System.out.println("Graph Name = " + graph_name);
                ...
                Memetico_TC_with_Poland meme = new Memetico_TC_with_Poland(logger, () -> true, problem, structSol, structPop, MetodoConstrutivo,
                ...
                graph_name,orig_length,orig_length_arr[count],(count+1));
            }//for
...
//      System.exit(1);
            ...
            System.out.println("error 556 = " + e.toString());
            e.printStackTrace();
//      System.exit(1);
        }
...
//                System.out.println();
    ...
    public double[][] array_to_matrix_polish(int tour_arr[]){
        int dimensions = tour_arr.length;
...
//                    System.out.println(w + ", and " + (w+1));
                    dist_arr[(tour_arr[w]-1)][(tour_arr[w+1]-1)] = 1;
                    dist_arr[(tour_arr[w+1]-1)][(tour_arr[w]-1)] = 1;
//                    count_check++;
                }else{
//                    System.out.println(w + ", and " + 0);
                    dist_arr[(tour_arr[w]-1)][(tour_arr[0]-1)] = 1;
                    dist_arr[(tour_arr[0]-1)][(tour_arr[w]-1)] = 1;
//                    count_check++;
                }
            }
